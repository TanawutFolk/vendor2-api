export const ClearTimeSQL = {
  ...
  updateIsCurrentClearTimeForSctProcess: async (dataItem: any) => {
    let sql = `
            UPDATE dataItem.CYCLE_TIME_DB.CLEAR_TIME_FOR_SCT_PROCESS
            SET IS_CURRENT = 0
    ...
    sql = sql.replaceAll('dataItem.PRODUCT_TYPE_ID', dataItem['PRODUCT_TYPE_ID'])
    sql = sql.replaceAll('dataItem.FISCAL_YEAR', dataItem['FISCAL_YEAR'])
    // console.log('1111111111111', sql)
    return sql
  },
  updateIsCurrentClearTimeForSctTotal: async (dataItem: any) => {
    let sql = `
            UPDATE dataItem.CYCLE_TIME_DB.CLEAR_TIME_FOR_SCT_TOTAL
            SET IS_CURRENT = 0
            WHERE PRODUCT_TYPE_ID = 'dataItem.PRODUCT_TYPE_ID'
            AND FISCAL_YEAR = 'dataItem.FISCAL_YEAR'
                        `
