  ...
  getBySctId: async (dataItem: { SCT_ID: string; IS_FROM_SCT_COPY: 0 | 1 | '' }) => {
    let sql = `
                SELECT
                        ...
                        , tb_2.SELLING_EXPENSE_RATE  AS SETTING_SELLING_EXPENSE_RATE
                        , tb_2.GA_RATE               AS SETTING_GA_RATE
                        , tb_2.MARGIN_RATE            AS SETTING_MARGIN_RATE
                        , tb_2.CIT                   AS SETTING_CIT
                        , tb_2.ADJUST_PRICE          AS SETTING_ADJUST_PRICE
                        , tb_2.INDIRECT_COST         AS SETTING_INDIRECT_COST
                        , tb_2.COST_CONDITION_SETTING_VERSION AS SETTING_VERSION
                        , tb_2.VAT                   AS SETTING_VAT
                        , tb_2.DIRECT_UNIT_PROCESS_COST AS SETTING_DIRECT_UNIT_PROCESS_COST
                        , tb_2.INDIRECT_RATE_OF_DIRECT_PROCESS_COST AS SETTING_INDIRECT_RATE_OF_DIRECT_PROCESS_COST
                        , tb_2.LEVEL_OF_INDIRECT_COST AS SETTING_LEVEL_OF_INDIRECT_COST
                FROM
                        dataItem.STANDARD_COST_DB.SCT_TOTAL_COST tb_1
                LEFT JOIN
                        dataItem.STANDARD_COST_DB.sct_cost_condition_setting scs
                            ON scs.SCT_ID = tb_1.SCT_ID AND scs.INUSE = 1
                LEFT JOIN
                        cost_condition_setting tb_2
                            ON tb_2.COST_CONDITION_SETTING_ID = scs.COST_CONDITION_SETTING_ID
                WHERE
