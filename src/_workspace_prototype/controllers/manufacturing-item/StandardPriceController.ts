...
export const StandardPriceController = {
  ...
  createByImportFile: async (req: Request, res: Response) => {
    ...
    if (result.Status === false) {
      ...
      if (result.ResultOnDb && result.ResultOnDb.length > 0) {
        result.ResultOnDb.forEach((item: any) => {
          const excelRowIndex = item.row

          if (columns[excelRowIndex]) {
            columns[excelRowIndex][0] = item.ITEM_CODE_FOR_SUPPORT_MES || ''

            // 🔥 รวม error หลายตัว
            const errorMessage = item.errors ? Object.values(item.errors).join(', ') : result.Message || ''
            columns[excelRowIndex][0] = item.ITEM_CODE_FOR_SUPPORT_MES
            // columns[excelRowIndex][1] = item.ITEM_CATEGORY_NAME || ''
            // columns[excelRowIndex][2] = item.ITEM_INTERNAL_FULL_NAME || ''
            // columns[excelRowIndex][3] = item.ITEM_INTERNAL_SHORT_NAME || ''
            columns[excelRowIndex][4] = item.PURCHASE_PRICE || ''
            columns[excelRowIndex][5] = item.PURCHASE_PRICE_CURRENCY_SYMBOL || ''
            columns[excelRowIndex][6] = item.PURCHASE_UNIT_CODE || ''
            columns[excelRowIndex][7] = item.USAGE_UNIT_RATIO || ''
            columns[excelRowIndex][8] = item.USAGE_UNIT_CODE || ''
            columns[excelRowIndex][9] = item.VERSION_NO || ''
            columns[excelRowIndex][10] = errorMessage
          }
        })
      }
