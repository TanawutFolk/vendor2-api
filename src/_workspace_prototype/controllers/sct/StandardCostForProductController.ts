...
export const StandardCostForProductController = {
  ...
  getAllWithWhereCondition_old_version: async (req: Request, res: Response) => {
    ...
    const createDateTimeFromFormatted = createDateTimeFrom && dayjs(createDateTimeFrom).isValid() ? dayjs(createDateTimeFrom).format('YYYY-MM-DD HH:mm:ss') : ''

    const createDateTimeTo = dataItem.SearchFilters.find((x: any) => x.id === 'createDateTimeTo')?.value
    const createDateTimeToFormatted = createDateTimeTo && dayjs(createDateTimeTo).isValid() ? dayjs(createDateTimeTo).format('YYYY-MM-DD HH:mm:ss') : ''
    const createDateColumn = "STR_TO_DATE(CREATE_DATE, '%d-%b-%Y %H:%i:%s')"

    if (createDateTimeFromFormatted) {
      dataItem.sqlWhere += !/where\s+/i.test(dataItem.sqlWhere)
        ? ` WHERE ${createDateColumn} >= '${createDateTimeFromFormatted}'`
        : ` AND ${createDateColumn} >= '${createDateTimeFromFormatted}'`
    }

    if (createDateTimeToFormatted) {
      dataItem.sqlWhere += !/where\s+/i.test(dataItem.sqlWhere)
        ? ` WHERE ${createDateColumn} <= '${createDateTimeToFormatted}'`
        : ` AND ${createDateColumn} <= '${createDateTimeToFormatted}'`
    }
