...
export const GetClearTimeExport = async (req: Request, res: Response) => {
  try {
    let dataItem
    ...
    for (let i = 0; i < dataArray.length; i++) {
      const dataDetail = await ClearTimeModel.getClearTimeExportDataByProductTypeId(dataArray[i])
    ...
    await workbookNFormula.xlsx
      ...
      .catch(function (err) {
        res.status(500).send({
          ...
          TotalCountOnDb: '',
          MethodOnDb: 'Search clear time Data',
...
async function writingFile(workbookNFormula: any) {
  const worksheet = workbookNFormula.getWorksheet('CLEAR TIME')
  ...
  for (let j = 0; j < dataResult.length; j++) {
    ...
    worksheet.getCell(2 + j, 15).value = element['CLEAR_TIME']
    worksheet.getCell(2 + j, 19).value = element['DESCRIPTION']
    worksheet.getCell(2 + j, 20).value = element['PROCESS_CAPACITY_SHEET_ID']
    worksheet.getCell(2 + j, 21).value = element['PROCESS_CAPACITY_SHEET_NAME']
    worksheet.getCell(2 + j, 22).value = element['APPROVED_DATE']
    worksheet.getCell(2 + j, 23).value = element['FISCAL_YEAR']
    worksheet.getCell(2 + j, 24).value = element['CLEAR_TIME_FOR_SCT']
    worksheet.getCell(2 + j, 25).value = element['REVISION_NO_CLEAR_TIME_FOR_SCT']
    // worksheet.getCell(2 + j, 16).value = element['LEAD_TIME'] == 1 ? 'O' : ''
    // worksheet.getCell(2 + j, 17).value = element['CYCLE_TIME'] == 1 ? 'O' : ''
    // worksheet.getCell(2 + j, 18).value = element['COLLECTION_POINT_FOR_SCT'] == 1 ? 'O' : ''
  }

  const leftColumns = [
    ...Array.from({ length: 15 }, (_, i) => i + 1), // A(1) - O(15
    17, // Q = 17
    ...Array.from({ length: 5 }, (_, i) => 21 + i), // U(21) - Y(25)
  ]
  leftColumns.forEach((col) => {
    worksheet.getColumn(col).alignment = { horizontal: 'left' }
  })

  const rightColumns = [16, 18, 19] // P=16, R=18, S=19
...
async function sendFile(res: Response, workbookNFormula: any) {
  ...
  const filename = `CLEAR-TIME-${year}${month}${day}-${hours}-${minutes}-${seconds}.xlsx`
