...
export const YieldRateImportService = {
  createYieldRate: async (dataItem: any) => {
    ...
    for (let i = 0; i < Object.keys(groupedByProductType)?.length; i++) {
      ...
      sqlList.push(await YieldRateAndGoStraightRateSQL.updateYieldRateProcessIsCurrent(element[0]))
      sqlList.push(await YieldRateAndGoStraightRateSQL.updateYieldRateTotalIsCurrent(element[0]))
      // ** Insert Total
