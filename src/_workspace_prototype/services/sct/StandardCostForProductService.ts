...
import { SctCostConditionSettingSQL } from '@src/_workspace/sql/sct/sct-for-product/SctCostConditionSettingSQL'
...
export const StandardCostForProductService = {
  search: async (
    ...
    const tableIds = [
      ...
      { table: 'tc', id: 'SELLING_EXPENSE_FOR_SELLING_PRICE', Fns: 'LIKE' },
      { table: '', id: 'GA_FOR_SELLING_PRICE', Fns: 'LIKE' },
      { table: '', id: 'MARGIN_FOR_SELLING_PRICE', Fns: 'LIKE' },
      { table: '', id: 'CIT_FOR_SELLING_PRICE', Fns: 'LIKE' },
      { table: 'tc', id: 'SELLING_PRICE_BY_FORMULA', Fns: 'LIKE' },
      { table: '', id: 'REMARK_FOR_ADJUST_PRICE', Fns: 'LIKE' },
    ]

    getSqlWhere_elysia(dataItem, tableIds, 'RE_CAL_UPDATE_DATE')
    ...
    if (dataItem.SearchFilters.find((item: any) => item.id == 'hasIndirectCost') !== '' && dataItem.SearchFilters.find((item: any) => item.id == 'hasIndirectCost') !== undefined) {
      if (dataItem.SearchFilters.find((item: any) => item.id == 'hasIndirectCost').value === 0) {
        dataItem.sqlWhere += dataItem.sqlWhere.includes('WHERE') ? ' AND TOTAL_INDIRECT_COST = 0' : ' WHERE TOTAL_INDIRECT_COST = 0'
      } else if (dataItem.SearchFilters.find((item: any) => item.id == 'hasIndirectCost').value === 1) {
        dataItem.sqlWhere += dataItem.sqlWhere.includes('WHERE') ? ' AND TOTAL_INDIRECT_COST > 0' : ' WHERE TOTAL_INDIRECT_COST > 0'
      }
    }
    ...
    //#region Re-cal update date filter
    const sctDateTimeFrom = dataItem.SearchFilters.find((x: any) => x.id === 'sctDateTimeFrom')?.value
    const sctDateTimeFrom_formatted = sctDateTimeFrom && dayjs(sctDateTimeFrom).isValid() ? dayjs(sctDateTimeFrom).format('YYYY-MM-DD HH:mm:ss') : ''

    const sctDateTimeTo = dataItem.SearchFilters.find((x: any) => x.id === 'sctDateTimeTo')?.value
    const sctDateTimeTo_formatted = sctDateTimeTo && dayjs(sctDateTimeTo).isValid() ? dayjs(sctDateTimeTo).format('YYYY-MM-DD HH:mm:ss') : ''

    if (sctDateTimeFrom_formatted) {
      dataItem.sqlWhere += dataItem.sqlWhere.includes('WHERE')
        ? " AND RE_CAL_UPDATE_DATE >= '" + sctDateTimeFrom_formatted + "'"
        : " WHERE RE_CAL_UPDATE_DATE >= '" + sctDateTimeFrom_formatted + "'"
    }

    if (sctDateTimeTo_formatted) {
      dataItem.sqlWhere += dataItem.sqlWhere.includes('WHERE')
        ? " AND RE_CAL_UPDATE_DATE <= '" + sctDateTimeTo_formatted + "'"
        : " WHERE RE_CAL_UPDATE_DATE <= '" + sctDateTimeTo_formatted + "'"
    }

    //#endregion Re-cal update date filter

    //#region Create date filter
    const createDateTimeFrom = dataItem.SearchFilters.find((x: any) => x.id === 'createDateTimeFrom')?.value
    const createDateTimeFrom_formatted = createDateTimeFrom && dayjs(createDateTimeFrom).isValid() ? dayjs(createDateTimeFrom).format('YYYY-MM-DD HH:mm:ss') : ''

    const createDateTimeTo = dataItem.SearchFilters.find((x: any) => x.id === 'createDateTimeTo')?.value
    const createDateTimeTo_formatted = createDateTimeTo && dayjs(createDateTimeTo).isValid() ? dayjs(createDateTimeTo).format('YYYY-MM-DD HH:mm:ss') : ''

    if (createDateTimeFrom_formatted) {
      dataItem.sqlWhere += /where\s+/i.test(dataItem.sqlWhere)
        ? " AND CREATE_DATE >= '" + createDateTimeFrom_formatted + "'"
        : " WHERE CREATE_DATE >= '" + createDateTimeFrom_formatted + "'"
    }

    if (createDateTimeTo_formatted) {
      dataItem.sqlWhere += /where\s+/i.test(dataItem.sqlWhere)
        ? " AND CREATE_DATE <= '" + createDateTimeTo_formatted + "'"
        : " WHERE CREATE_DATE <= '" + createDateTimeTo_formatted + "'"
    }

    //#endregion Create date filter
    ...
    if (typeof dataItem.SCT_ID === 'object') {
      for (let i = 0; i < dataItem.SCT_ID.length; i++) {
        ...
        if (dataItem.SCT_STATUS_PROGRESS_ID == 2) {
          ...
          // Delete sct_cost_condition_setting so that the next cal/re-cal
          // will pick up the latest setting version automatically
          sqlList.push(
            await SctCostConditionSettingSQL.deleteBySctId({
              SCT_ID: dataItem.SCT_ID[i],
            })
          )
        }
...
export async function getSctRevisionCodeByCondition(dataItem: GetSctRevisionCodeCondition) {
  ...
  let result: Input[] = await getParentProductTypeBySctRevisionCode(sctRevisionCode as Input[], dataItem)

  return result.map((item) => ({
    ...item,
    ROOT_FILTERED_SCT: sctRevisionCode.some((f) => f.SCT_REVISION_CODE === item.SCT_REVISION_CODE) ? 1 : 0,
  }))
}
type Input = {
  SCT_REVISION_CODE: string
...
async function getParentProductTypeBySctRevisionCode(dataItems: Input[], dataItem: GetSctRevisionCodeCondition, visited = new Set<string>()): Promise<SctItem[]> {
  const result: SctItem[] = []
  let sqlWhere = ''
  ...
  if (
    ...
    sqlWhere += ' AND tb_12.SCT_STATUS_PROGRESS_ID <> 1'
  }
  ...
  for (const code of dataItems) {
    ...
    const sql = await StandardCostForProductSQL.getParentProductTypeBySctRevisionCode({
      SCT_REVISION_CODE: code.SCT_REVISION_CODE,
      sqlWhere,
    })
    ...
    for (const parent of parents) {
      ...
      // ✅ FIX 2
      const upperParents = await getParentProductTypeBySctRevisionCode([parent], dataItem, visited)
