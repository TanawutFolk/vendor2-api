...
import { SctIndirectCostConditionSQL } from '@src/_workspace/sql/sct/sct-for-product/SctIndirectCostConditionSQL'
...
const processTask = async (
  ...
  if (sct.SCT_STATUS_PROGRESS_ID === 2 || sct.SCT_STATUS_PROGRESS_ID === 3) {
    ...
    const { FISCAL_YEAR, ITEM_CATEGORY_NAME, PRODUCT_MAIN_NAME, PRODUCT_MAIN_ID, ITEM_CATEGORY_ID, PRODUCT_TYPE_ID, PRODUCT_SUB_ID } = sct

    // TODO : get data from DB
    const { data: costCondition_Data, costConditionSettingId } = await _CostConditionService.getAllByProductMainIdAndFiscalYear_MasterDataLatest({
      FISCAL_YEAR,
      ITEM_CATEGORY_NAME,
      PRODUCT_MAIN_ID,
      PRODUCT_MAIN_NAME,
    ...
    const { BOM_ID, SCT_PATTERN_ID, PRODUCT_TYPE_CODE } = sct
    ...
    if (MaterialPrice_Data.some((item) => item.ITEM_IS_CURRENT === 0)) {
      throw new Error('PRODUCT_TYPE_CODE : ' + PRODUCT_TYPE_CODE + ', Item is not current')
    }
    ...
    for (const flowProcess of FlowProcess_Data) {
      ...
      if (isNaN(CLEAR_TIME) || CLEAR_TIME === undefined || YIELD_ACCUMULATION_FOR_SCT === undefined || GO_STRAIGHT_RATE === undefined) {
        throw new Error('CLEAR_TIME or YIELD_RATE or GO_STRAIGHT_RATE not found , PRODUCT_TYPE_CODE : ' + '-' + PRODUCT_TYPE_CODE)
      }
    ...
    // update : sct_cost_condition_setting (track which setting version was used)
    if (costConditionSettingId) {
      sqlList.push(await SctCostConditionSettingSQL.deleteBySctId({ SCT_ID: sct.SCT_ID }))
      sqlList.push(
        await SctCostConditionSettingSQL.insert({
          SCT_ID: sct.SCT_ID,
          COST_CONDITION_SETTING_ID: costConditionSettingId,
          CREATE_BY: sct.CREATE_BY,
          UPDATE_BY: sct.UPDATE_BY,
        })
      )
    ...
    if (!costCondition_Data[1][0].INDIRECT_COST_CONDITION_ID) {
      throw new Error('INDIRECT_COST_CONDITION_ID not found in : ' + sct.PRODUCT_TYPE_CODE)
    }

    sqlList.push(await SctIndirectCostConditionSQL.deleteBySctId({ SCT_ID: sct.SCT_ID, UPDATE_BY: sct.UPDATE_BY }))
    sqlList.push(
      await SctIndirectCostConditionSQL.insert({
        SCT_ID: sct.SCT_ID,
        INDIRECT_COST_CONDITION_ID: costCondition_Data[1][0].INDIRECT_COST_CONDITION_ID,
        CREATE_BY: sct.CREATE_BY,
        UPDATE_BY: sct.UPDATE_BY,
      })
    )
