...
export const YieldRateService = {
  search: async (dataItem: any) => {
    ...
    if (dataItem.includingCancelled == false) {
      sqlWhere += ' AND tb_5.INUSE = 1'
    }
    if (dataItem.yieldRateOption === 'Latest') {
      sqlWhere += ' AND tb_5.IS_CURRENT = 1'
    }
    if (dataItem.INUSE) {
      sqlWhere += " AND tb_5.INUSE = 'dataItem.INUSE' "
    }

    if (sqlWhere) {
      sqlWhere = ` WHERE ${sqlWhere.substring(4)}`
    }
    // sqlWhere = sqlWhere.substring(5) // ลบ AND ตัวแรกออก
  ...
  searchYieldRateProcess: async (dataItem: any) => {
    let sql = ''
  ...
  searchYieldRateTotal: async (dataItem: any) => {
    let sql = ''
  ...
  searchUnlimit: async (dataItem: any) => {
    let sql = ''
    ...
    if (dataItem.IS_MODE) {
      // ** Mode Total
      sql = await YieldRateSQL.searchYieldRateTotalUnlimit(dataItem, sqlWhere)
    } else {
      // ** Mode Process
      sql = await YieldRateSQL.searchYieldRateProcessUnlimit(dataItem, sqlWhere)
    }

    const resultData = (await MySQLExecute.search(sql)) as RowDataPacket[]
    return resultData
  },
  searchUnlimitYieldRate: async (dataItem: any) => {
    let sql = ''
  ...
  searchUnlimitYieldRateTotal: async (dataItem: any) => {
    let sql = ''
