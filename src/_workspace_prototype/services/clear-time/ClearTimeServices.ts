  ...
  createClearTime: async (dataItem: any) => {
    try {
      ...
      for (let i = 0; i < Object.keys(groupedByProductType)?.length; i++) {
        ...
        sqlList.push(await ClearTimeSQL.updateIsCurrentClearTimeForSctProcess(element[0]))
        sqlList.push(await ClearTimeSQL.updateIsCurrentClearTimeForSctTotal(element[0]))
