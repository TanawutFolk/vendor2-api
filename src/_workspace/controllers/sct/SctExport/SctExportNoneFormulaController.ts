...
let cntRowPricingNFormula: any

export const GetSctExportNoneFormula = async (req: Request, res: Response) => {
  try {
    let dataItem
    ...
    let dataArrayNFormula = dataItem['LIST_SCT_ID']
    ...
    const uniqueArray: Item[] = dataArrayNFormula.filter((obj: any, index: any, self: any) => index === self.findIndex((t: any) => t.SCT_ID === obj.SCT_ID))
    ...
    for (let i = 0; i < dataArrayNFormula.length; i++) {
      const dataHeaderPricing = await StandardCostForProductModel.getSctHeader(dataArrayNFormula[i])
      const dataHeaderPricingCompare = await StandardCostForProductModel.getSctHeaderCompare(dataArrayNFormula[i])
      const sellingPrice = await StandardCostForProductModel.getSellingPrice(dataArrayNFormula[i])
      const getSellingPriceCompare = await StandardCostForProductModel.getSellingPriceCompare(dataArrayNFormula[i])
      const indirectCostPriceForPricingTabsData = await StandardCostForProductModel.getIndirectCostPriceTabsDataForExport(dataArrayNFormula[i])
      const indirectCostPriceForPricingTabsDataCompare = await StandardCostForProductModel.getIndirectCostPriceTabsDataCompareForExport(dataArrayNFormula[i])

      if (dataHeaderPricing[0]) {
        dataHeaderPricing[0].LEVEL = dataArrayNFormula[i]?.LEVEL
      }
    ...
    await workbookNFormula.xlsx.readFile('Excel Cost Structure - None Formula Rev05.xlsx').then(async function () {
      //  ** Prepare Data
      copyWorksheetExcelFile(workbookNFormula, props).then(function () {
        createAllExcelFile(workbookNFormula, props)
          .then(function () {
            createPricingSheet(workbookNFormula, props).then(function () {
...
async function copyWorksheetExcelFile(workbookNFormula: any, props: any) {
  for (let i = 0; i < props.dataHeaderForExcel.length - 1; i++) {
    await copyWorksheet(i + 1, workbookNFormula)
  }
}

async function copyWorksheet(i: number, workbookNFormula: any) {
  var newWorksheet = workbookNFormula.getWorksheet('SCT_NAME_0')
  let copySheet = workbookNFormula.addWorksheet('copy')
...
async function createAllExcelFile(workbookNFormula: any, props: any) {
  for (let i = 0; i < props.dataHeaderForExcel.length; i++) {
    // ** SheetID , CountRow
    workbookNFormula = await writingFile(i, i, workbookNFormula, props)
  }
}
...
async function writingFile(sheetId: number, cntRows: number, workbookNFormula: any, props: any) {
  const worksheet = workbookNFormula.getWorksheet('SCT_NAME_' + `${sheetId}`)
  ...
  for (let i = 0; i < props.dataHeaderForExcel?.length; i++) {
    ...
    worksheet.getCell(19, 2).value = 'FFT' // elementIndirect['DIRECT_COST_CODE']
    worksheet.getCell(20, 2).value = elementIndirect['TOTAL_PROCESSING_TIME']
    worksheet.getCell(21, 2).value = elementIndirect['TOTAL_PROCESSING_TIME_INCLUDING_INDIRECT_RATE']

    worksheet.getCell(22, 2).value = elementIndirect['TOTAL_DIRECT_COST']
    worksheet.getCell(23, 2).value = elementIndirect['DIRECT_PROCESS_COST']
    worksheet.getCell(24, 2).value = elementIndirect['TOTAL_PRICE_OF_RAW_MATERIAL']
    worksheet.getCell(25, 2).value = elementIndirect['TOTAL_PRICE_OF_SUB_ASSY']
    worksheet.getCell(26, 2).value = elementIndirect['TOTAL_PRICE_OF_SEMI_FINISHED_GOODS']
    worksheet.getCell(27, 2).value = elementIndirect['TOTAL_PRICE_OF_CONSUMABLE']
    worksheet.getCell(28, 2).value = elementIndirect['TOTAL_PRICE_OF_PACKING']
    worksheet.getCell(29, 2).value = elementIndirect['TOTAL_PRICE_OF_ALL_OF_ITEMS']
    ...
    worksheet.getCell(32, 2).value = elementIndirect['TOTAL_PRICE_OF_ALL_OF_ITEMS_INCLUDE_IMPORTED_COST']
  ...
  // ***์ Compare Header
  // console.log('data', dataMaterial[cntRows]?.length)
  ...
  if (props.dataHeaderCompareForExcel[cntRows]?.length > 0) {
    for (let i = 0; i < props.dataHeaderCompareForExcel?.length; i++) {
      ...
      // ** Header
      worksheet.getCell(4, 3).value = Number(element['COMPARE_FISCAL_YEAR'])
      ...
      // worksheet.getCell(40, 2).value = elementIndirect['COMPARE_ASSEMBLY_GROUP_FOR_SUPPORT_MES']
      // worksheet.getCell(41, 3).value = elementIndirect['INDIRECT_COST_SALE_AVE']
    }
  }
...
async function sendFile(res: Response, workbookNFormula: any) {
  ...
  const filename = `SCT-NONE-FORMULA-${year}${month}${day}-${hours}-${minutes}-${seconds}.xlsx`
...
const createPricingSheet = async (workbookNFormula: any, props: any) => {
  // *** Set initial Rows
  cntRowPricingNFormula = 0
  // workbookNFormula = await writingPricingFile(workbookNFormula)
  await writingPricingFile(workbookNFormula, props)
}

async function writingPricingFile(workbookNFormula: any, props: any) {
  let worksheet = workbookNFormula.getWorksheet('Pricing')
  ...
  for (let k = 0; k < props.dataHeaderPricingForExcel.length; k++) {
    // console.log('product-Main', dataHeaderForExcel[k][0].PRODUCT_MAIN_NAME)
  ...
  if (outputMainName?.length <= 1) {
    worksheet.getCell(1 + cntRowPricingNFormula, 4).value = outputMainName[0]['FISCAL_YEAR']
    worksheet.getCell(1 + cntRowPricingNFormula, 8).value = outputMainName[0]['SCT_REASON_SETTING_NAME']
    worksheet.getCell(2 + cntRowPricingNFormula, 4).value = outputMainName[0]['PRODUCT_MAIN_NAME']
  } else {
    worksheet.getCell(1 + cntRowPricingNFormula, 4).value = null
    worksheet.getCell(1 + cntRowPricingNFormula, 8).value = null
    worksheet.getCell(2 + cntRowPricingNFormula, 4).value = null
  }
  // ** Writing Pricing File

  for (let i = 0; i < props.dataHeaderPricingForExcel.length; i++) {
    const element = output[i]
    recurse(element, worksheet)
  }
  return workbookNFormula
}

async function recurse(element: any, worksheet: any) {
  // internal link
  if (element) {
    if (element?.LEVEL === 0) {
      worksheet.getCell(5 + cntRowPricingNFormula, 2).value = element['NAME']
      // worksheet.getCell(5 + cntRowPricingNFormula, 2).font = { bold: true }
      worksheet.getCell(5 + cntRowPricingNFormula, 2).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!B${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 1) {
      worksheet.getCell(5 + cntRowPricingNFormula, 3).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 3).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!C${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 2) {
      worksheet.getCell(5 + cntRowPricingNFormula, 4).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 4).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!D${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 3) {
      worksheet.getCell(5 + cntRowPricingNFormula, 5).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 5).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!E${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 4) {
      worksheet.getCell(5 + cntRowPricingNFormula, 6).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 6).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!F${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 5) {
      worksheet.getCell(5 + cntRowPricingNFormula, 7).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 7).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!G${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 6) {
      worksheet.getCell(5 + cntRowPricingNFormula, 8).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 8).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!H${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 7) {
      worksheet.getCell(5 + cntRowPricingNFormula, 9).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 9).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!I${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 8) {
      worksheet.getCell(5 + cntRowPricingNFormula, 10).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 10).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!J${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 9) {
      worksheet.getCell(5 + cntRowPricingNFormula, 11).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 11).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!K${5 + cntRowPricingNFormula}`,
      }
    } else if (element?.LEVEL === 10) {
      worksheet.getCell(5 + cntRowPricingNFormula, 12).value = element['NAME']
      worksheet.getCell(5 + cntRowPricingNFormula, 12).value = {
        text: element['NAME'],
        hyperlink: `#${element['NAME']}!L${5 + cntRowPricingNFormula}`,
      }
    }
    worksheet.getCell(5 + cntRowPricingNFormula, 13).value = element['PRODUCT_TYPE_NAME']
    worksheet.getCell(5 + cntRowPricingNFormula, 14).value = element['ITEM_CATEGORY_NAME']
    worksheet.getCell(5 + cntRowPricingNFormula, 15).value = element['SCT_REVISION_CODE']
    worksheet.getCell(5 + cntRowPricingNFormula, 16).value = element['SCT_STATUS_PROGRESS_NAME']
    worksheet.getCell(5 + cntRowPricingNFormula, 17).value = element['DIRECT_UNIT_PROCESS_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 18).value = Number(element['INDIRECT_RATE_OF_DIRECT_PROCESS_COST'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 19).value = Number(element['IMPORTED_FEE'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 20).value = Number(element['SELLING_EXPENSE'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 21).value = Number(element['GA'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 22).value = Number(element['MARGIN'] / 100) ?? ''
    // worksheet.getCell(5 + cntRowPricing, 23).value = element['ITEM_CATEGORY_NAME'];
    // ** Direct Cost
    worksheet.getCell(5 + cntRowPricingNFormula, 24).value = element['RM_INCLUDE_IMPORTED_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 25).value = element['CONSUMABLE_PACKING']
    worksheet.getCell(5 + cntRowPricingNFormula, 26).value = element['MATERIALS_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 27).value = element['DIRECT_PROCESS_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 28).value = element['TOTAL_DIRECT_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 29).value = element['INDIRECT_COST_SALE_AVE']
    worksheet.getCell(5 + cntRowPricingNFormula, 30).value = element['SELLING_EXPENSE_FOR_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 31).value = element['GA_FOR_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 32).value = element['MARGIN_FOR_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 33).value = element['SELLING_PRICE_BY_FORMULA']
    worksheet.getCell(5 + cntRowPricingNFormula, 34).value = element['SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 35).value = moment(element['ESTIMATE_PERIOD_START_DATE']).format('DD-MMM-YY')
    worksheet.getCell(5 + cntRowPricingNFormula, 36).value = Number(element['TOTAL_YIELD_RATE'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 37).value = element['TOTAL_CLEAR_TIME']
    worksheet.getCell(5 + cntRowPricingNFormula, 38).value = element['ADJUST_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 39).value = Number(element['CIT'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 40).value = element['CIT_FOR_SELLING_PRICE']
  }
  // *** Compared Data
  if (element?.COMPARE_SCT_REVISION_CODE) {
    worksheet.getCell(5 + cntRowPricingNFormula, 44).value = element['COMPARE_SCT_REVISION_CODE']
    worksheet.getCell(5 + cntRowPricingNFormula, 45).value = element['COMPARE_SCT_STATUS_PROGRESS_NAME']
    worksheet.getCell(5 + cntRowPricingNFormula, 46).value = element['COMPARE_DIRECT_UNIT_PROCESS_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 47).value = Number(element['COMPARE_INDIRECT_RATE_OF_DIRECT_PROCESS_COST'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 48).value = Number(element['COMPARE_IMPORTED_FEE'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 49).value = Number(element['COMPARE_SELLING_EXPENSE'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 50).value = Number(element['COMPARE_GA'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 51).value = Number(element['COMPARE_MARGIN'] / 100) ?? ''

    worksheet.getCell(5 + cntRowPricingNFormula, 53).value = element['COMPARE_RM_INCLUDE_IMPORTED_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 54).value = element['COMPARE_CONSUMABLE_PACKING']
    worksheet.getCell(5 + cntRowPricingNFormula, 55).value = element['COMPARE_MATERIALS_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 56).value = element['COMPARE_DIRECT_PROCESS_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 57).value = element['COMPARE_TOTAL_DIRECT_COST']
    worksheet.getCell(5 + cntRowPricingNFormula, 58).value = element['COMPARE_INDIRECT_COST_SALE_AVE']
    worksheet.getCell(5 + cntRowPricingNFormula, 59).value = element['COMPARE_SELLING_EXPENSE_FOR_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 60).value = element['COMPARE_GA_FOR_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 61).value = element['COMPARE_MARGIN_FOR_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 62).value = element['COMPARE_SELLING_PRICE_BY_FORMULA']
    worksheet.getCell(5 + cntRowPricingNFormula, 63).value = element['COMPARE_SELLING_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 64).value = moment(element['COMPARE_ESTIMATE_PERIOD_START_DATE']).format('DD-MMM-YY')
    worksheet.getCell(5 + cntRowPricingNFormula, 65).value = Number(element['COMPARE_TOTAL_YIELD_RATE'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 66).value = element['COMPARE_TOTAL_CLEAR_TIME']
    worksheet.getCell(5 + cntRowPricingNFormula, 67).value = element['COMPARE_ADJUST_PRICE']
    worksheet.getCell(5 + cntRowPricingNFormula, 68).value = Number(element['COMPARE_CIT'] / 100) ?? ''
    worksheet.getCell(5 + cntRowPricingNFormula, 69).value = element['COMPARE_CIT_FOR_SELLING_PRICE']
  }
  cntRowPricingNFormula++
}
