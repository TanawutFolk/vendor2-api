...
const SearchFilterSchema = z.object({
    id: z.string(),
    value: z.union([z.string(), z.number(), z.boolean(), z.array(z.any()), z.null()]).optional().nullable()
})
...
export const UpdateVendorSchema = z.object({
    vendor_id: z.union([
        z.number().int().positive('Vendor ID is required'),
        z.string().regex(/^\d+$/, 'Vendor ID must be numeric')
    ]),
...
export const ExportVendorSchema = z.object({
    TYPE: z.enum(['currentPage', 'AllPage'], {
