...
export const normalizeText = (value: any) => String(value ?? '').replace(/\r/g, '\n').trim()
...
export const buildRowsPreview = (rows: any[][]) =>
    rows
        .slice(0, 5)
        .map(row => (row || []).slice(0, 10).map(cell => normalizeText(cell)))
...
export const GENERIC_HEADER_TOKENS = new Set([
    'NO', 'NUMBER', 'SOURCE', 'ENTITY', 'ENTITY NUMBER', 'ENTITY NO',
    'ENTITY TYPE', 'TYPE', 'PROGRAM', 'PROGRAMS', 'NAME', 'PRIMARY NAME',
    'COUNTRY', 'ALIAS', 'ALIASES', 'WMD',
])
...
export const hasDetectedHeader = (parsedRows: any) =>
    parsedRows?.__debug?.headerRow !== null && parsedRows?.__debug?.headerRow !== undefined
...
const findBestUsHeader = (rows: any[][]) => {
    ...
    let best: {
        rowIndex: number
        columns: {
            source: number; entity_number: number; entity_type: number; programs: number
            name: number; title: number; addresses: number; federal_register_notice: number
            start_date: number; end_date: number; standard_order: number
            license_requirement: number; license_policy: number; vessel_information: number
            remarks: number; source_list_url: number; alt_names: number; citizenships: number
            dates_of_birth: number; nationalities: number; places_of_birth: number
            source_information_url: number; description: number
            create_by: number; update_by: number; inuse: number
        }
        score: number
    } | null = null
...
const looksLikeUsHeaderRow = (
    sourceName: string, entityNumber: string, entityType: string,
    programs: string, primaryName: string,
) => {
...
export const parseUsRows = (worksheet: XLSX.WorkSheet): ParsedBlacklistUsRow[] => {
    ...
    const columns = bestHeader
        ? bestHeader.columns
        : {
            source: 0, entity_number: 1, entity_type: 2, programs: 3, name: 4,
            title: 5, addresses: 6, federal_register_notice: 7, start_date: 8,
            end_date: 9, standard_order: 10, license_requirement: 11,
            license_policy: 12, vessel_information: 13, remarks: 14,
            source_list_url: 15, alt_names: 16, citizenships: 17,
            dates_of_birth: 18, nationalities: 19, places_of_birth: 20,
            source_information_url: 21, description: 22,
            create_by: 23, update_by: 24, inuse: 25,
        }
    ...
    for (let rowIndex = startRow; rowIndex < rows.length; rowIndex++) {
        ...
        if (!sourceName && !entityNumber && !entityType && !programs && !primaryName) { skippedEmpty++; continue }
        if (looksLikeUsHeaderRow(sourceName, entityNumber, entityType, programs, primaryName)) { skippedHeader++; continue }
        if (!primaryName) { skippedNoName++; continue }
...
export const parseCnRows = (worksheet: XLSX.WorkSheet): ParsedBlacklistCnRow[] => {
    ...
    const columns = bestHeader
        ? bestHeader.columns
        : { no: 0, country: 1, primary_name: 2, aliases: 3, wmd_type: 4, extra: 5 }
    ...
    const parsedRows = rows
        ...
        .map<ParsedBlacklistCnRow | null>((row) => {
            ...
            const aliasLines = splitCellLines(safeRow[columns.aliases]).filter(
                alias => normalizeName(alias) !== normalizeName(primaryName),
            )
