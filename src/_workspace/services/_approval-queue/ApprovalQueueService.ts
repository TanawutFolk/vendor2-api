...
const getPendingAfterCurrent = (steps: ApprovalStep[], currentStep: ApprovalStep) => steps
    .filter(step => step.step_status === 'pending' && step.step_order > currentStep.step_order)
    .sort((a, b) => Number(a.step_order || 0) - Number(b.step_order || 0))

const getPendingNonBranchAnywhere = (steps: ApprovalStep[], currentStep: ApprovalStep) => steps
...
const loadWorkflowContext = async (dataItem: UpdateStatusPayload): Promise<WorkflowContext> => {
    const [stepsSql, checkSql, requesterSql] = await Promise.all([
        ApprovalQueueSQL.getApprovalSteps({ request_id: dataItem.request_id }),
        ApprovalQueueSQL.getRequestStatusContext({ request_id: dataItem.request_id }),
        ApprovalQueueSQL.getRequesterByRequestId({ request_id: dataItem.request_id }),
    ])
    const [stepsRes, checkRes, requesterRes] = await Promise.all([
        MySQLExecute.search(stepsSql) as Promise<ApprovalStep[]>,
        MySQLExecute.search(checkSql) as Promise<RequestRecord[]>,
        MySQLExecute.search(requesterSql) as Promise<RowDataPacket[]>,
    ])
...
const hasVendorRequestLog = async (context: WorkflowContext) => {
    ...
    return logs.some(log =>
        String(log.step_id || '') === String(context.currentStep?.step_id || '') && log.action_type === 'vendor_requested'
    )
}
...
const addVendorCodeUpdates = async (context: WorkflowContext, sqlList: SqlList) => {
    ...
    sqlList.push(await ApprovalQueueSQL.updateRequestVendorCode({
    ...
    if (vendor_id) {
        sqlList.push(await ApprovalQueueSQL.updateVendorFftVendorCode({
            vendor_id,
            vendor_code: dataItem.vendor_code_extracted,
        }))
    }
...
const handleRejection = async (context: WorkflowContext, sqlList: SqlList, postCommitTasks: PostCommitTask[]) => {
    ...
    if (currentStep) {
        sqlList.push(await ApprovalQueueSQL.updateApprovalStep({
            step_id: currentStep.step_id,
            step_status: 'rejected',
            UPDATE_BY: dataItem.UPDATE_BY || dataItem.approve_by || 'SYSTEM',
        }))
        sqlList.push(await ApprovalQueueSQL.createApprovalLog({
            request_id: dataItem.request_id,
            step_id: currentStep.step_id,
...
const handleVendorReplyRequest = async (
    ...
    const isNegotiationBranchCurrentStep = isStepType(
        currentStep,
        StepType.PENDING_AGREEMENT,
        StepType.AGREEMENT_REACHED,
    ...
    if (pendingAgreementStep) {
        ...
        if (pendingAgreementApprover && pendingAgreementApprover !== pendingAgreementStep.approver_id) {
            sqlList.push(await ApprovalQueueSQL.updateApprovalStepApprover({
...
const handleGprCRequesterPhase = async (
    ...
    const isRequesterGprCPhase =
        getStepType(currentStep) === StepType.ISSUE_GPR_C
        && !!requesterCode
    ...
    await runPostCommitTasks(postCommitTasks, dataItem.request_id)
...
const handleNormalApproval = async (
    ...
    if (nextStep) {
        ...
        sqlList.push(await ApprovalQueueSQL.markRequestCompleted({
            request_id: dataItem.request_id,
            UPDATE_BY: dataItem.UPDATE_BY || dataItem.approve_by || 'SYSTEM',
        }))
        if (actionRequiredRequested) {
            ...
            if (context.steps.length > 0) {
                if (isRejectedStatus(newStatus) || isExplicitReject) {
                    ...
                    const disagreementRequested =
                        ...
                        isIssueGprBStatus(newStatus) ||
                        isIssueGprCStatus(newStatus)
                    const gprCRequesterResponse = await handleGprCRequesterPhase(
                        context,
                        resolver,
                        sqlList,
                        postCommitTasks,
                    ...
                    await handleNormalApproval(context, resolver, sqlList, postCommitTasks, explicitAction, newStatus)
                }
            }
    ...
    reassignAssignment: async (dataItem: ReassignPayload) => {
        try {
            ...
            if (scope === 'REQUEST_PIC') {
                ...
                sqlList.push(await ApprovalQueueSQL.updateRequestPicAssignee({
                    request_id: requestId,
            ...
            sqlList.push(await ApprovalQueueSQL.insertAssignmentHistory({
                request_id: requestId,
