...
export const ApprovalQueueService = {
    ...
    updateStatus: async (dataItem: any) => {
        try {
            ...
            const stepsSql = await ApprovalQueueSQL.getApprovalSteps({ request_id: dataItem.request_id })
            const steps = (await MySQLExecute.search(stepsSql)) as RowDataPacket[]
            const currentStep = steps.find((s: any) => s.step_status === 'in_progress')
            ...
            if (currentStep && requiresVendorReply(currentStep)) {
                const logsSql = await ApprovalQueueSQL.getApprovalLogs({ request_id: dataItem.request_id })
            ...
            const checkSql = await ApprovalQueueSQL.getRequestStatusContext({ request_id: dataItem.request_id })
            ...
            const requesterSql = await ApprovalQueueSQL.getRequesterByRequestId({ request_id: dataItem.request_id })
            ...
            const getApproverByGroup = async (groupCode: string) => {
                if (!groupCode) return ''
                const approverSql = await ApprovalQueueSQL.getApproverByGroupCode({ group_code: groupCode })
            ...
            const getSelectionRecord = async () => {
                ...
                const selectionSql = await ApprovalQueueSQL.getSelection({ request_id: dataItem.request_id })
            ...
            const sqlList = []
            sqlList.push(await ApprovalQueueSQL.updateStatus(dataItem))
