...
export const AddVendorService = {
    checkBlacklist: async (dataItem: any) => {
        ...
        const blacklistResult = normalizedName
            ? await MySQLExecute.search(BlacklistSQL.checkBlacklist(normalizedName)) as RowDataPacket[]
            : []
        ...
        return {
            ...
            Message: blacklistResult.length > 0
                ? `Vendor name matches ${blacklistResult.length} record(s) in the Blacklist`
    ...
    checkDuplicateVendor: async (dataItem: any) => {
        ...
        const [duplicateResult, blacklistResult] = await Promise.all([
            ...
            normalizedName
                ? MySQLExecute.search(BlacklistSQL.checkBlacklist(normalizedName)) as Promise<RowDataPacket[]>
