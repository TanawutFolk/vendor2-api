        ...
        try {
            ...
            const cleanName = (raw: string) => {
                ...
                return raw.toString().toLowerCase()
                    .replace(/co\.,?ltd\.?/g, '').replace(/limited/g, '').replace(/ltd\.?/g, '')
                    .replace(/company/g, '').replace(/part\.,?ltd\.?/g, '').replace(/หจก\./g, '')
                    .replace(/บจก\./g, '').replace(/บริษัท/g, '').replace(/\s/g, '')
                    .replace(/[^a-z0-9ก-๙]/g, '')
            }
            const calculateSimilarity = (str1: string, str2: string): number => {
                if (!str1 || !str2) return 0
                const tokenize = (text: string) => text.toString().toLowerCase().replace(/[^a-z0-9ก-๙\s]/g, ' ').split(/\s+/).filter(w => w.length > 2)
                const tokens1 = tokenize(str1), tokens2 = tokenize(str2)
                let matches = 0
            ...
            for (const vendor of vendors) {
                const mTel = cleanTel(vendor.tel_center), mName = cleanName(vendor.company_name), mAddress = vendor.address || ''
                let bestScore = 0, bestMethods: string[] = [], bestPronesRef: any = null
                for (const prones of pronesRows) {
                    let score = 0, methods: string[] = []
                    if (mName && cleanName(prones.customer_name) === mName) { score++; methods.push('name') }
                    if (mTel && cleanTel(prones.customer_tel) === mTel) { score++; methods.push('tel') }
                    if (mAddress) {
                        const pFullAddr = `${prones.customer_address1 || ''} ${prones.customer_address2 || ''} ${prones.customer_address3 || ''}`
                        if (calculateSimilarity(mAddress, pFullAddr) > 0.7) { score++; methods.push('address') }
                    }
                    if (score > bestScore) { bestScore = score; bestMethods = methods; bestPronesRef = prones }
