// Shared by request registration, approval, history, and task pages.
  ...
  gprCSelectionFields: (selectionAlias = 'rvs', requestAlias = 'rr') => {
    let sql = `
                                     , COALESCE((
                                           ...
                                           WHERE f.REQUEST_REGISTER_VENDOR_ID = dataItem.REQUEST_ALIAS.REQUEST_REGISTER_VENDOR_ID
                                             AND f.INUSE = 1
                                     ...
                                     , COALESCE((
                                           ...
                                           FROM (
                                               ...
                                               WHERE REQUEST_VENDOR_SELECTIONS_ID = dataItem.SELECTION_ALIAS.REQUEST_VENDOR_SELECTIONS_ID
                                                 AND INUSE = 1
                                     ...
                                     , COALESCE((
                                           ...
                                           WHERE aset.REQUEST_VENDOR_SELECTIONS_ID = dataItem.SELECTION_ALIAS.REQUEST_VENDOR_SELECTIONS_ID
                                             AND aset.INUSE = 1
                                       ), JSON_OBJECT('_meta', JSON_OBJECT('gpr_c_approver_empcode', '', 'gpr_c_pc_pic_empcode', ''))) AS ACTION_REQUIRED_JSON
                                     , dataItem.SELECTION_ALIAS.GPR_B_FILE_PATH AS GPR_B_FILE_PATH
                                     , dataItem.SELECTION_ALIAS.GPR_B_FILE_NAME AS GPR_B_FILE_NAME
`
    sql = sql.replaceAll('dataItem.REQUEST_ALIAS', String(requestAlias))
    sql = sql.replaceAll('dataItem.SELECTION_ALIAS', String(selectionAlias))
    return sql
  },
}
