...
try {
  ...
  assert(statusRows.length === 13, `Expected 13 active statuses, received ${statusRows.length}`)
  assert(
    statusRows.some((row) => row.stepCode === 'REJECTED' && row.workflowStepId === null),
    'Rejected terminal status is missing from status options'
  )

  const approvalSteps = await search(
    await RequestRegisterPageSQL.getApprovalSteps({ REQUEST_ID: 145 })
  )
  assert(approvalSteps.length === 12, `Expected 12 approval steps, received ${approvalSteps.length}`)
  assert(
    approvalSteps.every((row) => Number(row.WORKFLOW_STEP_ID || row.workflow_step_id) > 0),
    'Approval step is missing WORKFLOW_STEP_ID'
  )

  const requestRows = await search(await ApprovalQueueSQL.getById({ REQUEST_ID: 145 }))
  ...
  const [taskCountSql, taskDataSql] = await TaskManagerSQL.searchAllTask({
    SEARCHFILTERS: [],
    COLUMNFILTERS: [],
  ...
  const integrityRows = await search(`
    SELECT
      (SELECT COUNT(*) FROM request_approval_step WHERE WORKFLOW_STEP_ID IS NULL) AS unlinked_steps,
      (
        SELECT COUNT(*)
        ...
        WHERE rr.REQUEST_ID IS NULL OR vc.VENDOR_CONTACT_ID IS NULL
      ) AS orphan_contacts,
      (
        SELECT COUNT(*)
        FROM request_vendor_selections s
        LEFT JOIN request_register_vendor rr ON rr.REQUEST_ID = s.REQUEST_ID
  ...
  const selectionRows = await search(
    RequestRegisterPageSQL.getSelection({ REQUEST_ID: 145 })
  )
  const selectionId = Number(selectionRows[0]?.SELECTION_ID || selectionRows[0]?.selection_id || 0)
  const actionSetupRows = selectionId
    ? await search(RequestRegisterPageSQL.getGprActionSetup({ SELECTION_ID: selectionId }))
    : []
  const circularMemberRows = selectionId
    ? await search(RequestRegisterPageSQL.getGprCircularMembers({ SELECTION_ID: selectionId }))
    : []
  ...
  try {
    ...
    const firstStepId = Number(approvalSteps[0]?.STEP_ID || approvalSteps[0]?.step_id || 0)
    ...
    const vendorTypeRows = await search(
      'SELECT VENDOR_TYPE_ID FROM master_vendor_types WHERE INUSE = 1 LIMIT 1'
    )
    const productGroupRows = await search(
      'SELECT PRODUCT_GROUP_ID FROM master_product_groups WHERE INUSE = 1 LIMIT 1'
    )
    const [vendorInsertResult] = await connection.query(await AddVendorSQL.createVendor({
      COMPANY_NAME: 'ZZ AUDIT SMOKE ROLLBACK',
      PROVINCE: 'BANGKOK',
      POSTAL_CODE: '00000',
      VENDOR_TYPE_ID: Number(vendorTypeRows[0]?.VENDOR_TYPE_ID || 0),
      VENDOR_REGION: 'Local',
    ...
    await connection.query(await AddVendorSQL.createVendorProduct({
      VENDOR_ID: vendorId,
      PRODUCT_GROUP_ID: Number(productGroupRows[0]?.PRODUCT_GROUP_ID || 0),
    ...
    await connection.query(FindVendorSQL.insertMatchResultBatch([{
      VENDOR_ID: Number(requestRows[0].VENDOR_ID || 0),
      STATUS_CHECK: 'smoke',
      PRONES_CODE: 'SMOKE',
